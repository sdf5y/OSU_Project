...
OH_Month_series = OH_Month_series.dropna(subset= 'Year') #nas in the year 2025.
...
OH_Month_series = OH_Month_series[['Series ID', 'STATE', 'STATEFP', 'COUNTYNAME', 'COUNTYFP', 'Month',
       'Year',  'Indicator', 'Value', 'Population']]   #[['Series ID', 'STATE', 'STATEFP', 'COUNTYNAME', 'COUNTYFP', 'Month','Year', 'Date', 'Indicator', 'Value', 'Population']]
...
data = json.dumps({"seriesid": unemployment_rate[0],"startyear":"2020", "endyear":"2024"})
...
# %%
#Make a copy of the dataframe (before removing NAs)
OH_Month_series_test = OH_Month_series.copy()

data = OH_Month_series_test.sort_values(by=['COUNTYFP', 'Year'])


# %%
#make means across the 2022 to 2024 years for each county.
mean_pop = (data[(data['Year'] >= 2022) & (data['Year'] <= 2024)].groupby('COUNTYFP')['Population'].mean().reset_index().rename(columns={'Population': 'Mean_Pop_2022_2024'}))

data = data.merge(mean_pop, on='COUNTYFP', how='left')


# %%
#mask imputated rows
mask_2025_na = (data['Year'] == 2025) & (data['Population'].isna())
data.loc[mask_2025_na, 'Population'] = data.loc[mask_2025_na, 'Mean_Pop_2022_2024']

#put back the mask.
data['Pop_imp'] = False
data.loc[mask_2025_na, 'Pop_imp'] = True

data = data.drop(columns=['Mean_Pop_2022_2024'])
